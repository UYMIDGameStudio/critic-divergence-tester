...
**0.2.14** 修复多行表格单元格导出后拆行的问题，保留换行、边界空白与公式反斜线，
并转义单元格里的 HTML 样式文字。见 [0.2.14 修复记录](docs/release-engineering-0.2.14.md)，
