...
_GATE_LIFECYCLE = GateLifecycle(
    label="Gate B",
    corpus_artifact="product-gate-b-corpus",
    assessment_artifact="product-gate-b-assessment",
    decision_artifact="product-gate-b-decision",
    assessment_pattern=ASSESSMENT_PATTERN,
    decision_pattern=DECISION_PATTERN,
    validator=validate_artifact,
    readiness=lambda root: gate_b_readiness(root),
...
def initialize_gate_b(output: Path | str, projects: list[Path | str]) -> GateBPaths:
    ...
    _GATE_LIFECYCLE.initialize(
        ...
        build_report=rebuild_gate_b_report,
    )
    return paths
...
def append_gate_b_assessment(root: Path | str, project_alias: str, *, lineage_correction_minutes: int, lineage_reasonable: str, split_merge_worked: str, finding_inheritance_correct: str, resolved_stopped_reappearing: str, unresolved_persisted: str, revision_rationale_clarity: str, notes: str) -> Path:
    paths = gate_b_paths(root); corpus, corpus_bytes = _GATE_LIFECYCLE.read_corpus(paths.corpus, read_json=_read_json)
    ...
    _GATE_LIFECYCLE.validate_new(value, kind="assessment")
...
def gate_b_readiness(root: Path | str) -> list[str]:
    paths = gate_b_paths(root); corpus, _ = _GATE_LIFECYCLE.read_corpus(paths.corpus, read_json=_read_json); assessments = _entries(paths.assessments, ASSESSMENT_PATTERN)
    issues: list[str] = []
...
def rebuild_gate_b_report(root: Path | str):
    paths = gate_b_paths(root); record, record_bytes, markdown = _derive_report(paths)
    changed = _GATE_LIFECYCLE.rebuild_report(
        ((paths.report_record, record_bytes), (paths.report_markdown, markdown)),
...
def verify_gate_b(root: Path | str) -> list[str]:
    ...
    try:
        ...
        decisions = _entries(paths.decisions, DECISION_PATTERN)
        errors.extend(_GATE_LIFECYCLE.decision_chain_errors(decisions, digest=sha256_bytes))
        for _, decision, data in decisions:
